...
public class AccountsController : ControllerBase
{
    ...
    public Task<AccountDto> Update(Guid id, [FromBody] UpdateAccountRequest request) =>
        _mediator.Send(new UpdateAccountCommand(
            id, request.Name, request.Balance, request.LastFourDigits, request.IconKey, request.IconColor));
...
public record UpdateAccountRequest(
    string? Name, decimal? Balance, string? LastFourDigits, string? IconKey, string? IconColor);
